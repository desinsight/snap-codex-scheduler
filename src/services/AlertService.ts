...
export class AlertService {
  ...
  private async sendSlack(webhookUrl: string, channel: string, message: string): Promise<void> {
  ...
  private async sendWebhook(url: string, headers: Record<string, string>, payload: any): Promise<void> {
    await axios.post(url, payload, { headers });
  }

  private async sendInApp(notification: AlertNotification): Promise<void> {
  ...
  // 채널 설정 관리
  async setChannelConfig(channel: AlertChannel, config: AlertChannelConfig): Promise<void> {
    await axios.post(`/api/alerts/channels/${channel}/config`, config);
    this.channelConfigs.set(channel, config);
  }

  async getChannelConfig(channel: AlertChannel): Promise<AlertChannelConfig | undefined> {
  ...
  private formatSlackMessage(notification: AlertNotification): string {
