...
export const createTestCacheManager = (optimizer) => {
    return new AdvancedCacheManager(optimizer);
};
// 캐시 정리 대기
export const waitForCacheCleanup = (ms = 1000) => {
    return new Promise(resolve => setTimeout(resolve, ms));
};
// 대용량 데이터 생성
export const generateLargeData = (sizeInKB) => {
    const data = {};
...
// API 호출 모의
export const mockApiCall = async (delay = 100) => {
...
export class TestMemoryMonitor {
    ...
    simulateMemoryIncrease(bytes) {
        this.usage += bytes;
    }
    simulateMemoryDecrease(bytes) {
