...
// 서비스 최적화 인스턴스 생성
const optimizer = new ServiceOptimizer<User | User[] | UserProfile>('UserService', cacheConfig);
...
export const UserService = {
  ...
  getUsers: async (forceRefresh = false): Promise<User[]> => {
    return optimizer.withOptimization<User[]>(
      'users-all',
      async () => {
  ...
  getUserById: async (userId: string, forceRefresh = false): Promise<User> => {
    return optimizer.withOptimization<User>(
      `user-${userId}`,
      async () => {
  ...
  createUser: async (user: Omit<User, 'userId'>): Promise<User> => {
    return optimizer.withOptimization<User>(
      'user-create',
      async () => {
  ...
  // Profile operations
  getUserProfile: async (userId: string, forceRefresh = false): Promise<UserProfile> => {
    return optimizer.withOptimization<UserProfile>(
      `profile-${userId}`,
      async () => {
  ...
  // Password operations
  changePassword: async (userId: string, passwords: { oldPassword: string; newPassword: string }): Promise<void> => {
