export class NodeGraphService {
    ...
    async getNodeGraphData(timeRange) {
        try {
            // 시간 범위에 따른 필터링
            const now = new Date();
            ...
            const filteredLinks = this.data.links.filter(link => nodeIds.has(link.source.id) && nodeIds.has(link.target.id));
            return {
                nodes: filteredNodes,
                links: filteredLinks
            };
    ...
    async deleteNode(id) {
        try {
            ...
            // 관련 링크도 삭제
            this.data.links = this.data.links.filter(link => link.source.id !== id && link.target.id !== id);
        }
        catch (error) {
