...
export const fetchTaskById = createAsyncThunk('tasks/fetchById', async (taskId) => {
    return await TaskService.getTaskById(taskId);
});
export const updateTaskStatus = createAsyncThunk('tasks/updateStatus', async ({ id, status, note }) => {
    return await TaskService.updateTaskStatuses([{ id, status, note }]);
});
export const addTaskNote = createAsyncThunk('tasks/addNote', async ({ id, note }) => {
...
export const selectAllTasks = (state) => state.tasks.tasks.ids.map(id => state.tasks.tasks.entities.tasks[id]);
export const selectTaskById = (state, taskId) => state.tasks.tasks.entities.tasks[taskId];
export const selectTaskNotes = (state, id) => {
